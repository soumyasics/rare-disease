...
function Viewsinglepatientforcgatcoun() {
    const { id } = useParams();
    ...
    const Counsellorid = localStorage.getItem("counsellorlogin");
          ...
          <div className="chat-main-headbox">
            <div className="ri-arrow-left-line d-flex chat-mainbox-arrow" onClick={navbackfn}>
              <div className="chat-mainbox-image d-flex">
                <img src={`${url}/${patient?.image?.filename}`} alt="Doctor"/>
                <p>{patient?.name}</p>
              </div>
            </div>
          </div>
...
export default Viewsinglepatientforcgatcoun