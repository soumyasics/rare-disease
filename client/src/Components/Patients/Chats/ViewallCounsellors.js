...
function ViewallCounsellors() {
  ...
  const url = axiosInstance.defaults.url;
    ...
    const fetchAllRescueMembers = () => {
      axiosInstance.post(`viewCounsellorBookingByPatientid/${id}`)
          .then((res) => {
              console.log(res)
              const filteredDatas = res.data.data.reduce((uniquePatients, currentPatient) => {
                ...
                if (!uniquePatients.some(patient => patient.counsellorId._id === currentPatient.counsellorId._id)) {
  ...
  const handleSearch = (e) => {
    ...
    if (value.trim() === '') {
        ...
        const filteredData = data.filter(a => 
            a.counsellorId.name.toLowerCase().includes(value.toLowerCase())
        )
      ...
      <div className="view-chat-allcounse-container">
        ...
        <div className="container chatviewallcounsellor-content">
          ...
          <>
            <div className="col-3">
              <p>
                <img src={`${url}/${e?.counsellorId?.image?.filename}`} /> {e?.counsellorId?.name}
              </p>
            </div>
            <div className="col-3">
              <p>{e?.counsellorId?.regno}</p>
            </div>
            <div className="col-3">
...
export default ViewallCounsellors;
